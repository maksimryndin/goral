...
fn alerts_cpu_rule(
    ...
    if messenger.is_none() && alert_if_cpu_usage_percent_more_than.is_some() {
        return Err(serde_valid::validation::Error::Custom(
            "`[resources].messenger` should be specified if `[resources].alert_if_cpu_usage_percent_more_than` is specified".to_owned(),
        ));
    }
    Ok(())
}
...
fn alerts_disk_rule(
    ...
    if messenger.is_none() && alert_if_free_disk_space_percent_less_than.is_some() {
        return Err(serde_valid::validation::Error::Custom(
            "`[resources].messenger` should be specified if `[resources].alert_if_free_disk_space_percent_less_than` is specified".to_owned(),
        ));
    }
    Ok(())
}
...
fn alerts_memory_rule(
    ...
    if messenger.is_none() && alert_if_free_memory_percent_less_than.is_some() {
        return Err(serde_valid::validation::Error::Custom(
            "`[resources].messenger` should be specified if `[resources].alert_if_free_memory_percent_less_than` is specified".to_owned(),
        ));
    }
    Ok(())
}
